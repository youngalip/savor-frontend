  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-4">
        ...
        <span className="px-3 py-1 rounded-full text-xs font-semibold border bg-pink-50 text-pink-700 border-pink-200">
          Pastry
        </span>
      </div>
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          <div className="card p-4 mb-6 border border-cream-200 bg-cream-50 rounded-lg">
            ...
            <p className="text-sm text-pink-600 font-medium mt-1">{getTimeElapsed(order.orderTime)}</p>
          </div>
          ...
          {allItemsDone && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6 flex gap-3 items-start">
              ...
              <div>
                ...
                <p className="text-sm text-green-800">Semua pastry telah selesai dibuat.</p>
              </div>
            </div>
          )}
              ...
              <div key={item.id} className="card p-4 border border-cream-200 rounded-lg">
                <div className="flex justify-between mb-3">
                  <div>
                    ...
                    {item.notes && (
                      <p className="text-sm text-pink-600 mt-1">📝 {item.notes}</p>
                    )}
                  </div>
                </div>
...
const PastryStationContent = () => {
  ...
  const stationType = 'pastry'; // Pastry station
  ...
  return (
    <div className="flex min-h-screen bg-cream-50">
      ...
      <div className={`flex-1 transition-all duration-300 ${isCollapsed ? 'lg:ml-0' : 'lg:ml-64'}`}>
        ...
        <div className="bg-white border-b border-cream-200 px-8 py-6 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-4 py-2 bg-pink-50 rounded-lg">
              <span className="text-2xl font-bold text-pink-600">{getTotalPendingItems()}</span>
              <span className="text-sm text-pink-600 font-medium">Item Pending</span>
            </div>
            <button
              onClick={() => refetch()}
              disabled={isFetching}
        ...
        {/* Status Bar */}
        <div className="bg-pink-50 border-b border-pink-200 px-8 py-2 flex items-center justify-between text-sm">
          <div className="flex items-center gap-2 text-pink-700">
            <Wifi size={16} />
            <span>Last update: {lastUpdatedSeconds}s ago • Auto-refresh every 30s</span>
          </div>
          {isFetching && <span className="text-pink-600">Updating...</span>}
        </div>
