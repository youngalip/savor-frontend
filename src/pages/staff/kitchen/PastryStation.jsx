...
// --- Dummy Orders (Pastry Only) ---
const initialPastryOrders = [
  {
    id: 'ORD-001',
    tableNumber: 5,
    ...
    status: 'processing',
    items: [
      { id: 'item-1', name: 'Croissant', category: 'pastry', quantity: 2, price: 25000, status: 'processing', notes: 'Hangat' },
      { id: 'item-2', name: 'Chocolate Cake', category: 'pastry', quantity: 1, price: 35000, status: 'processing', notes: '' }
    ],
    totalAmount: 85000
  },
  {
    ...
    status: 'processing',
    items: [
      { id: 'item-3', name: 'Apple Pie', category: 'pastry', quantity: 1, price: 28000, status: 'done', notes: 'Dengan es krim' }
    ],
    totalAmount: 28000
  },
  {
    ...
    status: 'processing',
    items: [
      { id: 'item-4', name: 'Kue Lapis', category: 'pastry', quantity: 3, price: 20000, status: 'processing', notes: '' },
      { id: 'item-5', name: 'Donut', category: 'pastry', quantity: 6, price: 12000, status: 'processing', notes: 'Mix variant' }
    ],
    totalAmount: 132000
  },
  {
    ...
    status: 'processing',
    items: [
      { id: 'item-6', name: 'Cheese Tart', category: 'pastry', quantity: 4, price: 18000, status: 'done', notes: '' }
    ],
    totalAmount: 72000
  }
];
...
const OrderCard = ({ order, onViewDetails }) => {
  ...
  return (
    <div className="card p-5 hover:shadow-md transition-shadow cursor-pointer bg-white border border-cream-200 rounded-xl" onClick={() => onViewDetails(order)}>
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="text-lg font-bold text-gray-900">Meja {order.tableNumber}</h3>
          <p className="text-sm text-gray-500">#{order.id}</p>
        </div>
        <span className="px-3 py-1 rounded-full text-xs font-semibold border bg-pink-50 text-pink-700 border-pink-200">Pastry</span>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <span className="px-3 py-1 rounded-full text-xs font-medium border bg-blue-50 text-blue-700 border-blue-200">Diproses: {processingItems.length}</span>
        <span className="px-3 py-1 rounded-full text-xs font-medium border bg-green-50 text-green-700 border-green-200">Siap: {doneItems.length}</span>
      </div>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-cream-200 p-6 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Kelola Pesanan Pastry</h2>
          ...
          {allItemsDone && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6 flex gap-3 items-start">
              ...
              <div>
                ...
                <p className="text-sm text-green-800">Semua pastry dalam pesanan ini sudah selesai dibuat.</p>
              </div>
            </div>
          )}
...
const PastryStationContent = () => {
  const [orders, setOrders] = useState(initialPastryOrders);
  const [filteredOrders, setFilteredOrders] = useState(initialPastryOrders);
    ...
    <div className="flex min-h-screen bg-cream-50">
      ...
      <div className={`flex-1 transition-all duration-300 ${isCollapsed ? 'lg:ml-0' : 'lg:ml-64'}`}>
        ...
        <div className="bg-white border-b border-cream-200 px-8 py-6 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Pastry Station</h1>
            <p className="text-gray-500 mt-1">Pesanan roti dan kue yang sedang diproses</p>
          </div>
          <div className="flex items-center gap-2 px-4 py-2 bg-pink-50 rounded-lg">
            <span className="text-2xl font-bold text-pink-600">{getTotalProcessingItems()}</span>
            <span className="text-sm text-pink-600 font-medium">Item Diproses</span>
          </div>
        </div>
        ...
        <div className="bg-white border-b border-cream-200 px-8 py-4">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium ${tableFilter === 'all' ? 'bg-pink-500 text-white' : 'bg-cream-100 text-gray-700 hover:bg-cream-200'}`}
          >
            Semua Meja
          </button>
          {tableNumbers.map((table) => (
            <button
              key={table}
              onClick={() => setTableFilter(table.toString())}
              className={`px-4 py-2 ml-2 rounded-lg text-sm font-medium ${tableFilter === table.toString() ? 'bg-pink-500 text-white' : 'bg-cream-100 text-gray-700 hover:bg-cream-200'}`}
            >
              Meja {table}
            </button>
          ))}
        ...
        <div className="p-8">
          {filteredOrders.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Tidak ada pesanan pastry</h3>
              <p className="text-gray-500">Saat ini tidak ada pesanan roti dan kue yang perlu diproses</p>
            </div>
          ) : (
...
const PastryStation = () => (
  <SidebarProvider>
    <PastryStationContent />
  </SidebarProvider>
);

export default PastryStation;
