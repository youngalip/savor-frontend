...
const ProcessingOrders = () => {
  ...
  const [orders, setOrders] = useState(processingOrders);
  const [filteredOrders, setFilteredOrders] = useState(processingOrders);
  ...
  const handleStatusUpdate = (orderId, newStatus) => {
    ...
    // Remove from this list if status changed
    const stillProcessing = updatedOrders.filter(order => order.status === 'processing');
    setOrders(stillProcessing);
    setFilteredOrders(stillProcessing);
  };
      ...
      `}>
        ...
        <div className="bg-white border-b border-cream-200 px-8 py-6 mt-16 lg:mt-0">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Pesanan Diproses</h1>
              <p className="text-gray-500 mt-1">Pesanan yang sedang dikerjakan dapur</p>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-lg">
              <span className="text-2xl font-bold text-blue-600">{filteredOrders.length}</span>
              <span className="text-sm text-blue-600 font-medium">Pesanan Aktif</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-8">
          {filteredOrders.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Tidak ada pesanan diproses</h3>
              <p className="text-gray-500">Saat ini tidak ada pesanan yang sedang diproses dapur</p>
            </div>
          ) : (
